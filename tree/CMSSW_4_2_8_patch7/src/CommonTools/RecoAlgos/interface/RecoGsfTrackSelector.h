#ifndef RecoSelectors_RecoGsfTrackSelector_h
#define RecoSelectors_RecoGsfTrackSelector_h
/* \class RecoGsfTrackSelector
 *
...
class RecoGsfTrackSelector {
 public:
  typedef reco::GsfTrackCollection collection;
  typedef std::vector<const reco::GsfTrack *> container;
  typedef container::const_iterator const_iterator;

  /// Constructors
  RecoGsfTrackSelector() {}
  RecoGsfTrackSelector ( const edm::ParameterSet & cfg ) :
  ...
  RecoGsfTrackSelector ( double ptMin, double minRapidity, double maxRapidity, double minAbsEta, double maxAbsEta,
    ...
    minAbsEta_( minAbsEta ), maxAbsEta_( maxAbsEta ), 
  ...
  void select( const edm::Handle<collection>& c, const edm::Event & event, const edm::EventSetup&) {
    ...
    for( reco::GsfTrackCollection::const_iterator trk = c->begin(); 
         trk != c->end(); ++ trk )
  ...
  /// Operator() performs the selection: e.g. if (recoGsfTrackSelector(track)) {...}
  bool operator()( const reco::GsfTrack & t, const reco::BeamSpot* bs) {
    ...
    return
      (t.hitPattern().trackerLayersWithMeasurement() >= minHit_ &&
       ...
       fabs(t.eta()) >= minAbsEta_ && fabs(t.eta()) <= maxAbsEta_ &&
