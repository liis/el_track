...
AnalyticalGsfTrackSelector::AnalyticalGsfTrackSelector( const edm::ParameterSet & cfg ) :
...
AnalyticalGsfTrackSelector::~AnalyticalGsfTrackSelector() {
}

void AnalyticalGsfTrackSelector::produce( edm::Event& evt, const edm::EventSetup& es ) 
{
  using namespace std; 
  using namespace edm;
  using namespace reco;

  Handle<GsfTrackCollection> hSrcTrack;
  //BM Handle< vector<Trajectory> > hTraj;
  //BM Handle< vector<Trajectory> > hTrajP;
  //BM Handle< TrajTrackAssociationCollection > hTTAss;
  ...
  selGsfTracks_ = auto_ptr<GsfTrackCollection>(new GsfTrackCollection());
  rGsfTracks_ = evt.getRefBeforePut<GsfTrackCollection>();      
  if (copyExtras_) {
    selGsfTrackExtras_ = auto_ptr<TrackExtraCollection>(new TrackExtraCollection());
    selGsfTrackGsfExtras_ = auto_ptr<GsfTrackExtraCollection>(new GsfTrackExtraCollection());
    ...
    rGsfTrackExtras_ = evt.getRefBeforePut<TrackExtraCollection>();
    rGsfTrackGsfExtras_ = evt.getRefBeforePut<GsfTrackExtraCollection>();
  }
  ...
  // Loop over tracks
  size_t current = 0;
  for (GsfTrackCollection::const_iterator it = hSrcTrack->begin(), ed = hSrcTrack->end(); it != ed; ++it, ++current) {
    const GsfTrack & trk = * it;
    ...
    selGsfTracks_->push_back( GsfTrack( trk ) ); // clone and store
    if (ok && setQualityBit_) {
      selGsfTracks_->back().setQuality(qualityToSet_);
      if (!points.empty()) {
	if (qualityToSet_==TrackBase::loose) selGsfTracks_->back().setQuality(TrackBase::looseSetWithPV);
	else if (qualityToSet_==TrackBase::highPurity) selGsfTracks_->back().setQuality(TrackBase::highPuritySetWithPV);
      }
    }
    if (copyExtras_) {
      // TrackExtras
      selGsfTrackExtras_->push_back( TrackExtra( trk.outerPosition(), trk.outerMomentum(), trk.outerOk(),
						 trk.innerPosition(), trk.innerMomentum(), trk.innerOk(),
						 trk.outerStateCovariance(), trk.outerDetId(),
						 trk.innerStateCovariance(), trk.innerDetId(),
						 trk.seedDirection(), trk.seedRef() ) );
      selGsfTrackGsfExtras_->push_back( *(trk.gsfExtra()) );
      

      selGsfTracks_->back().setExtra( TrackExtraRef( rGsfTrackExtras_, selGsfTrackExtras_->size() - 1) );
      selGsfTracks_->back().setGsfExtra( GsfTrackExtraRef( rGsfTrackGsfExtras_, selGsfTrackGsfExtras_->size() - 1) );

      TrackExtra & tx = selGsfTrackExtras_->back();
	...
	tx.add( TrackingRecHitRef( rHits_, selHits_->size() - 1) );
      ...
      /// --------- HERE I HAVE TO ADD THE GSF EXTRA
    }
    //BM if (copyTrajectories_) {
    //BM   trackRefs_[current] = TrackRef(rTracks_, selTracks_->size() - 1);
    //BM }
  }
  //BM 
  /*
  ...
  */
  ...
  evt.put(selGsfTracks_);
  if (copyExtras_ ) {
    evt.put(selGsfTrackExtras_); 
    evt.put(selGsfTrackGsfExtras_); 
    evt.put(selHits_);
  }
...
bool AnalyticalGsfTrackSelector::select(const reco::BeamSpot &vertexBeamSpot, const reco::GsfTrack &tk, const std::vector<Point> &points) {
...
void AnalyticalGsfTrackSelector::selectVertices(const reco::VertexCollection &vtxs, std::vector<Point> &points) {
