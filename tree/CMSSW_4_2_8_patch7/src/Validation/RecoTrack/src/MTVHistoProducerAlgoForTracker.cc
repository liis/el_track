...
void MTVHistoProducerAlgoForTracker::bookRecoHistos(){
  ...
  */
  ...
  // hit-finding efficiency/purity
  hitPurity_vs_eta.push_back( dbe_->book2D("hitPurity_vs_eta","hitPurity vs eta",nintEta,minEta,maxEta,50,0.5,1.0));
  hitFindingEff_vs_eta.push_back( dbe_->book2D("hitFindingEff_vs_eta","hitFinding efficiency vs eta",nintEta,minEta,maxEta,50,0.5,1.0));
...
void MTVHistoProducerAlgoForTracker::fill_ResoAndPull_recoTrack_histos(int count,
  ...
  const reco::GsfTrack* gsfTrack(0);
  if(useGsf){
    gsfTrack = dynamic_cast<const reco::GsfTrack*>(&track);
  ...
  double etares=track.eta()-etaSim;
  ...
  //cout << "event: " << tp.eventId().bunchCrossing() << " , " << tp.eventId().event()  << endl;

  if((*TpSelectorForEfficiencyVsEta)(tp)){
    // --- MY DEBUG
    /*
    double fbremSim = fabs( (tp.g4Tracks().back().momentum().pt() - tp.g4Tracks().front().momentum().pt())/
			    tp.g4Tracks().front().momentum().pt());
    */
    /*
    double fbremSim(0);
    if(tp.g4Tracks().size()>1)
      fbremSim = fabs( (tp.g4Track_begin()+1)->momentum().pt() - tp.g4Track_begin()->momentum().pt()  )/
	tp.g4Track_begin()->momentum().pt() ;
    
    cout << "count matched " << endl;    
    if(fbremSim>0.95) cout << "count BREM95" << endl;
    if(fbremSim>0.90) cout << "count BREM90" << endl;
    if(fbremSim>0.80) cout << "count BREM80" << endl;
    if(fbremSim>0.70) cout << "count BREM70" << endl;
    if(fbremSim>0.50) cout << "count BREM50" << endl;
    if(fbremSim>0.30) cout << "count BREM30" << endl;
    */
    ...
    if( fabs( (ptRec-ptSim)/ptSim)>0.95  )
      cout << "count BREM95" << endl;

    if( fabs( (ptRec-ptSim)/ptSim)>0.5  )
      cout << "count BREM50" << endl;

    if( fabs( (ptRec-ptSim)/ptSim)>0.3  )
      cout << "count BREM30" << endl;
    */


    /*
    if( fabs(dxyRec-dxySim)>1){
      cout << "tp status, pdgId, pt, eta, d0: " 
	   << tp.status() << " , " << tp.pdgId() 
	   << " , " << tp.pt()   << " , " << tp.eta()
	   << " , " << dxySim << endl;
      cout << "tk pt, eta, d0: " << track.pt() << " , " << track.eta() << " , " << dxyRec << endl;
      cout << "chargeTP,track.charge(): " << chargeTP << " , " << track.charge() << endl;
      cout << "tp.g4Tracks.size, ptFirst, ptLast: " << tp.g4Tracks().size() 
	   << " , " << tp.g4Tracks().front().momentum().pt()
	   << " , " << tp.g4Tracks().back().momentum().pt() << endl;
    }
    */
    // ----

    //resolution of track params: fill 2D histos
    etares_vs_eta[count]->Fill(getEta(etaSim),etares);
    dxyres_vs_eta[count]->Fill(getEta(etaSim),dxyRec-dxySim);
    ptres_vs_eta[count]->Fill(getEta(etaSim),(ptRec-ptSim)/ptSim);
    dzres_vs_eta[count]->Fill(getEta(etaSim),dzRec-dzSim);
    phires_vs_eta[count]->Fill(getEta(etaSim),phiRec-phiSim);
    cotThetares_vs_eta[count]->Fill(getEta(etaSim),1/tan(M_PI*0.5-lambdaRec)-1/tan(M_PI*0.5-lambdaSim));

    //pulls of track params vs eta: fill 2D histos
    dxypull_vs_eta[count]->Fill(getEta(etaSim),dxyPull);
    ptpull_vs_eta[count]->Fill(getEta(etaSim),ptres/ptError);
    dzpull_vs_eta[count]->Fill(getEta(etaSim),dzPull);
    phipull_vs_eta[count]->Fill(getEta(etaSim),phiPull);
    thetapull_vs_eta[count]->Fill(getEta(etaSim),thetaPull);
  }
	
  if((*TpSelectorForEfficiencyVsPt)(tp)){
    //same as before but vs pT
    dxyres_vs_pt[count]->Fill(getPt(ptSim),dxyRec-dxySim);
    ptres_vs_pt[count]->Fill(getPt(ptSim),(ptRec-ptSim)/ptSim);
    dzres_vs_pt[count]->Fill(getPt(ptSim),dzRec-dzSim);
    phires_vs_pt[count]->Fill(getPt(ptSim),phiRec-phiSim);
    cotThetares_vs_pt[count]->Fill(getPt(ptSim),1/tan(M_PI*0.5-lambdaRec)-1/tan(M_PI*0.5-lambdaSim));  	 
  }

  if((*TpSelectorForEfficiencyVsPhi)(tp)){
    //plots vs phi
    nhits_vs_phi[count]->Fill(phiSim,track.numberOfValidHits());
    chi2_vs_phi[count]->Fill(phiSim,track.normalizedChi2());
    ptmean_vs_eta_phi[count]->Fill(phiSim,getEta(etaSim),ptRec);
    phimean_vs_eta_phi[count]->Fill(phiSim,getEta(etaSim),phiRec);
    ptres_vs_phi[count]->Fill(phiSim,(ptRec-ptSim)/ptSim);
    phires_vs_phi[count]->Fill(phiSim,phiRec-phiSim); 
...
void 
MTVHistoProducerAlgoForTracker::fill_hitFinding_histos(int count,
						       const TrackingParticle& tp,
						       ParticleBase::Vector momentumTP,
						       ParticleBase::Point vertexTP,
						       int chargeTP,
						       int nSimHits, int nSharedHits, int nRecoTrackHits,
						       const reco::Track& track)
{
  //std::cout << "shared, sim, rec: " << nSharedHits << " , " << nSimHits << " , " << nRecoTrackHits << std::endl;
  hitPurity_vs_eta[count]->Fill(momentumTP.eta(),(double) nSharedHits/nRecoTrackHits);
  hitFindingEff_vs_eta[count]->Fill(momentumTP.eta(),(double) nSharedHits/nSimHits);
