...
def initialize_histogram_2D(var, bin_reg_1, bin_reg_2 ):
    if len(bin_reg_1) == 3: # assume const. binning
        ...
        hist = ROOT.TH2F(var, var, bin_reg_1[0], bin_reg_1[1], bin_reg_2[0], bin_reg_2[1], bin_reg_2[2])
    else:
        "Wrongly initialized histograms, please fix bin_reg and bin_type to allowed values"
        sys.exit()
    return hist
...
def initialize_histograms(vars, bin_reg, hist_in_regions = [], dim = "1D" ):
    """
    ...
    for var in vars:
        if len(hist_in_regions) != 0:
            for region in hist_in_regions:
                if dim == "2D":
                    histograms[var + "_" + region] = initialize_histogram_2D(var + "_" + region, bin_reg, vars[var])
                else:
